...
def main():
    ...
    pretax = st.number_input("PreTax 401(k) ($)", 0, 10_000_000, pretax_default, step=50_000, format="%d")
            roth_bal = st.number_input("Roth IRA ($)", 0, 5_000_000, file_balances["roth"] if file_balances["roth"] is not None else 510_000, step=25_000, format="%d")
            hsa_bal = st.number_input("HSA ($)", 0, 500_000, file_balances["hsa"] if file_balances["hsa"] is not None else 130_000, step=10_000, format="%d")
            s5_bal = st.number_input("S+ 5-Year Payout ($)", 0, 2_000_000, file_balances["s_plus_5"] if file_balances["s_plus_5"] is not None else 300_000, step=25_000, format="%d")
            s10_bal = st.number_input("S+ 10-Year Payout ($)", 0, 2_000_000, file_balances["s_plus_10"] if file_balances["s_plus_10"] is not None else 400_000, step=25_000, format="%d")
            cash_bal = st.number_input("Cash ($)", 0, 1_000_000, file_balances["cash"] if file_balances["cash"] is not None else 745_000, step=10_000, format="%d")
